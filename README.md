...
- `map_and_company/map_integration.py`：现有 8767 地图能力演示服务。
...
已提供可选的局域网共享覆盖配置 `docker-compose.shared.yml`、TLS 网关、文件扫描器与 `.env.shared.example`；**默认 `start.cmd` 仍仅供本机使用**。启用前必须轮换所有演示账户密码、设置随机数据库口令和 JWT 密钥，并完成客户端证书信任与防火墙限制。具体步骤与尚需人工验收的项目见 [`docs/本轮指定问题修复与部署验收.md`](docs/本轮指定问题修复与部署验收.md)。
...
登录和会话恢复不再向空库隐式写入演示客户；集团管理员需在“数据集成管理 → 演示数据迁移”预览并确认，未改动的批次可回滚。周经分只显示服务端授权数据且明细分页；线索池、文档也支持翻页。文档提供实际格式校验、版本列表、历史下载和恢复；共享环境需配置可用的 ClamAV 扫描服务，扫描不可用时上传失败。审计入口合并集成事件和跨模块 API 写入/失败事件，以 traceId 关联；不会记录请求正文或 API Key。
