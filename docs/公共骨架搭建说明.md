...
1. **演示路径**：`data/智拓商机作战助手-开发版.html` + `src/ZhiTuoNativeServer.py` + `map_and_company/map_integration.py`，继续用于现阶段页面演示和需求确认。
...
特别提醒：当前仓库中的 `map_and_company/TenCentApiKey.txt` 已经被 Git 跟踪。新增 `.gitignore` 不能自动取消历史跟踪。团队应尽快轮换该密钥，并在确认新环境变量配置可用后执行：

```powershell
git rm --cached -- "map_and_company/TenCentApiKey.txt"
